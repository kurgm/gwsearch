...
- グリフ項目：グリフウィキの1実体グリフを表します。別名（エイリアス）グリフがあれば，「=」に続けて列挙されます。各リンクをクリックすると対応するグリフページが開きます。グリフ項目の1つ下の階層には次のグリフ項目があります。
    + そのグリフを直接引用するグリフ
    + そのグリフの旧版を直接引用するグリフ
...
- DAGの各頂点は各実体グリフとします。
- グリフAが，グリフB（実体）またはBの別名グリフを直接引用している場合，辺B→AをDAGに追加します。
